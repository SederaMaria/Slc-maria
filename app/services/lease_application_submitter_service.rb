class LeaseApplicationSubmitterService
  ...
  validates :lease_calculator_us_state,
            :lease_calculator_tax_jurisdiction,
  ...
  # validates :lease_calculator_tax_jurisdiction,
  #           presence: true,
  #           if: -> { georgia_state}
  ...
  def submit!(**options)
    if(application&.lessee&.home_address&.state != 'GA')
  ...
  def georgia_state
    return lessee_home_address_state.present? && lessee_home_address_state != 'GA'
  end
