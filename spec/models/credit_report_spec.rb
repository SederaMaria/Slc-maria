...
RSpec.describe CreditReport, type: :model do
  let(:report) { create(:credit_report) }
  let(:audit) { report.audits.last }

  it { is_expected.to be_simple_audited }

  context '#create' do
    it 'creates audits on create' do
      expect(audit.audited_changes).to eql("#{bold('Created')} this Credit Report with #{bold('Status')} set to #{bold('pending')}")
    end
  end

  describe '#upload' do
    it 'has S3 uploader' do
      expect(report.upload.class).to eq S3Uploader
    end
  end
end
